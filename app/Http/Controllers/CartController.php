...
class CartController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach (Cart::session(auth()->user()->id)->getContent() as $item) {
            ...
            if($item->attributes->inc_certificate){
                $inc_cert_ids[] = $item->id;
            }
        }
        ...
        $certs_total = number_format((Cart::session(auth()->user()->id)->getTotal()-$total),2);
        return view('frontend.cart.checkout', compact('courses', 'bundles', 'total', 'taxData', 'certs_total', 'inc_cert_ids'));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($request->has('course_id')) {
            ...
            if($request->inc_certificate){
                $product->price = $product->price+$product->cert_price;
            }
            ...
            Cart::session(auth()->user()->id)
                ->add($product->id, $product->title, ($product->price), 1,
                    [
                        ...
                        'inc_certificate' => ($request->inc_certificate ? 1 : 0),
                        'cert_price' => (($request->inc_certificate) ? ($product->cert_price ?? 0) : 0),
...
//        return view('frontend.cart.checkout', compact('courses', 'total', 'taxData'));
    ...
    {   
        ...
        if ($request->course_id) {
            ...
            $course = Course::where('free', true)->findOrFail($id);
            if($request->inc_certificate && $course->cert_price > 0){
                return $this->checkout($request);
            }
        } else {
            $type = Bundle::class;
            $id = $request->bundle_id;
            $bundle = Bundle::where('free', true)->findOrFail($id);
        }
    ...
    {   
        ...
        foreach (Cart::session(auth()->user()->id)->getContent() as $cartItem) {
            ...
            $order->items()->create([
                ...
                'cert_price' => $cartItem->attributes->cert_price
            ]);
        }
//        Cart::session(auth()->user()->id)->removeConditionsByType('coupon');
        return $order;
