...
//        $request = $this->saveFiles($request);
        ...
        if ($request->hasFile('avatar_location')) {
            ...
            $teacher->avatar_location = $request->avatar_location->store('/avatars', 'public');
        }
    ...
    public function edit($id)
    {
        ...
        $payment_details = $teacher->teacherProfile->payment_details;
        return view('backend.teachers.edit', compact('teacher', 'payment_details'));
    }
...
//        $request = $this->saveFiles($request);
        ...
        if($request->has('password')){
            ...
            if($request->hasFile('personal_photo')){
                $docs_data['personal_photo'] = $request->file('personal_photo')->store('/docs', 'public');
            }
            if($request->hasFile('passport_photo')){
                $docs_data['passport_photo'] = $request->file('passport_photo')->store('/docs', 'public');
            }
            if($request->hasFile('academic_degree_photo')){
                $docs_data['academic_degree_photo'] = $request->file('academic_degree_photo')->store('/docs', 'public');
            }
            if($request->hasFile('cv')){
                $docs_data['cv'] = $request->file('cv')->store('/docs', 'public');
            }
            if($teacher->docs && $docs_data){
                UserDoc::where('user_id', $teacher->id)->update($docs_data);
            }else{
                $docs_data['user_id'] = $teacher->id;
                UserDoc::create($docs_data);
            }
        }


        return redirect()->route('admin.teachers.edit', $teacher->id)->withFlashSuccess(trans('alerts.backend.general.updated'));
    }
