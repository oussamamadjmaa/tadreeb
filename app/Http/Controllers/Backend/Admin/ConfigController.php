...
class ConfigController extends Controller
{
    ...
    public function getGeneralSettings()
    {
        ...
        $default_cert_data = [
            'show_e3tmad' => 1,
            'text_1' => 'تشهد منصة '.config('app.name').' و',
            'text_2' => 'بأن السيد/(ة)',
            'text_3' => 'قد اجتاز/(ت) الدورة التدريبية :',
            'text_4' => 'وأنهى/(ت)',
            'text_5' => 'للمدرب :'
        ];
        $cert_data = Config::firstOrCreate(
            ['key' => 'cert_data'],
            ['value' => json_encode($default_cert_data)]
        );
        $cert_data = json_decode($cert_data->value);
        ...
        return view('backend.settings.general', compact('sections', 'footer_data','app_locales','api_clients', 'cert_data'));
    }
    ...
    public function saveGeneralSettings(Request $request)
    {
        ...
        foreach ($requests->all() as $key => $value) {
            ...
            if ($key != '_token') {
                ...
                if($key == "cert_data"){
                    $value = $value;
                    $value['show_e3tmad'] = (isset($request->show_e3tmad)) ? 1 : 0;
                    $config->value = $value;
                }else{
                    $config->value = $value;
                }
