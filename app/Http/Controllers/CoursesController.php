...
class CoursesController extends Controller
{
    ...
    public function all()
    {
        ...
        return view('frontend.courses.index', compact('courses', 'purchased_courses', 'recent_news', 'featured_courses', 'categories'));
    }
    public function flagemail(Request $request,Course $course){
        abort_if(!$course->is_expired, 404);
        $request->validate([
            'email' => 'required|email:dns'
        ]);
        Flagemail::firstOrCreate([
            'course_id' => $course->id,
            'email' => $request->email
        ]);
        return redirect()->route('courses.show', [$course->slug])->with(['success' => __('global.flagemail.success')]);
    }
    public function show($course_slug)
    {
        ...
        return view('frontend.courses.course', compact('course', 'purchased_course', 'recent_news', 'course_rating', 'completed_lessons', 'total_ratings', 'is_reviewed', 'lessons', 'continue_course', 'checkSubcribePlan','courseInPlan'));
    }
    ...
    public function getByCategory(Request $request)
    {
        ...
        if ($category != "") {
            ...
            return view('frontend.courses.index', compact('courses', 'category', 'recent_news', 'featured_courses', 'categories'));
        }
        return abort(404);
    }
