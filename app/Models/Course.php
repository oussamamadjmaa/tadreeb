...
//    protected $dates = ['expire_at'];
    ...
    public function e3tmads()
    {
        return $this->belongsToMany(User::class, 'course_user')->withPivot('e3tmad_id');
    }
    ...
    public function scopeOfE3tmad($query)
    {
        if (!Auth::user()->isAdmin()) {
            return $query->whereHas('e3tmads', function ($q) {
                $q->where('e3tmad_id', Auth::user()->id);
            });
        }
        return $query;
    }
    ...
    {
        return (is_null($this->expire_at) || $this->expire_at < Carbon::now()->format('Y-m-d'));
    }
    ...
    public function getBagTypeTextAttribute(){
        return (($this->bag_type == 2) ? trans('labels.backend.courses.fields.Sync') : trans('labels.backend.courses.fields.Asynchronous'));
    }
    public function e3tmad(){
        return $this->hasOne(User::class, 'id', 'e3tmad_id');
    }

    public function can_get_certificate(){
        if(auth()->check()){
            if($this->cert_price == 0){
                return true;
            }
            $order_item = OrderItem::whereHas('order', fn($q) => $q->where('user_id', \Auth::id())->where('status', 1))->where('item_id', $this->id)->where('item_type', Course::class)->first();
            return ($order_item && $order_item->cert_price > 0);
        }
        return false;
    }
}
