            ...
            <div class="row">
                <div class="col-sm-2 col-xs-12">
                    ...
                    <div>
                        ...
                        <span class="type">{{$course->start_date}}</span>
                    </div>
                </div>
                <div class="col-sm-2 col-xs-12">
                    <div>
                        <i class="fas fa-clock "></i>
                        <span class="title">{{trans('labels.backend.courses.fields.duration')}}</span>
                        <span class="type">{{$course->duration ?? 'لا يوجد'}}</span>
                    </div>
                </div>
                <div class="col-sm-2 col-xs-12">
                    <i class="fas fa-check-double"></i>
                    <div>
                        <span class="title">{{trans('labels.backend.courses.fields.voltage')}} </span>
                        <span class="type">{{$course->voltage ?? 'لا يوجد'}}</span>
                    </div>
                </div>
                <div class="col-sm-2 col-xs-12">
                    <i class="fas fa-sliders-h"></i>
                    <div>
                        <span class="title">{{trans('labels.backend.courses.fields.level')}} </span>
                ...
                <div class="course-details-item border-bottom-0 mb-0">
                    ...
                    <div class="course-single-text">
                        <div class="course-title mt10 headline relative-position">
                            <h4 class="main-text"><a href="{{ route('courses.show', [$course->slug]) }}"><b>{{$course->title}}</b></a>
                                @if($course->trending == 1)
                                <span class="trend-badge text-uppercase bold-font"><i class="fas fa-bolt"></i> @lang('labels.frontend.badges.trending')</span>
                                @endif
                                @if($course->free == 1)
                        ...
                        @if ($course->goals)
                        <div class="course-title mt10 headline relative-position">
                            <h4 class="main-text"><b>{{trans('labels.backend.courses.fields.goals')}}</b></h4>
                        </div>
                        <div class="course-details-content">
                            {!! $course->goals !!}
                        </div>
                        ...
                        <div class="course-title mt10 headline relative-position">
                            <h4 class="main-text"><b>{{trans('labels.backend.courses.fields.requirements')}}</b></h4>
                        </div>
                        <div class="course-details-content">
                            {!! $course->requirements !!}
                        </div>
                        ...
                        @if($course->outputs)
                        <div class="course-title mt10 headline relative-position">
                            <h4 class="main-text"><b>{{trans('labels.backend.courses.fields.outputs')}}</b></h4>
                        </div>
                        <div class="course-details-content">
                            {!! $course->outputs !!}
                        </div>
                        ...
                        <div class="course-title mt10 headline relative-position">
                            <h4 class="main-text"><b>{{trans('labels.backend.courses.fields.target_group')}}</b></h4>
                        </div>
                        <div class="course-details-content">
                            {!! $course->target_group !!}
                        </div>
                        ...
                        <div class="course-title mt10 headline relative-position">
                            <h4 class="main-text"><b>{{trans('labels.backend.courses.fields.sponsor_name')}}</b></h4>
                        </div>
                        <div class="course-details-content">
                            {!! $course->sponsor_name !!}
                        </div>
                            ...
                            <div class="panel position-relative">
                                ...
                                <div id="collapse{{$count}}" class="collapse" aria-labelledby="headingOne" data-parent="#accordion">
                                    <div class="panel-body">
                                        ...
                                        @if(in_array($lesson->model->id,$completed_lessons))
                                        <div>
                                            <a class="btn btn-warning mt-3" href="{{route('lessons.show',['course_id' => $lesson->course->id,'slug'=>$lesson->model->slug])}}">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="best-course-pic-text relative-position">
                                            <div class="best-course-pic relative-position" @if($bundle->course_image != "") style="background-image: url('{{asset('storage/uploads/'.$course->course_image)}}')" @endif>
                                            ...
                                            <div class="best-course-text">
                                                ...
                                                <div class="course-meta">
                                                    <span class="course-category"><a href="{{route('courses.category',['category'=>$bundle->category->slug])}}">{{$bundle->category->name}}</a></span>
                ...
                <div class="side-bar">
                    <div class="course-side-bar-widget">
                        ...
                        @if (auth()->check() && (auth()->user()->hasRole('student')) && !(Cart::session(auth()->user()->id)->get($course->id)))
                            <input type="checkbox" id="inc_certificate_" name="inc_certificate_" style="float: right;">
                        @endif
                        <h5 style="float: right;text-align: right;margin-right: 5px;margin-top: -7px;color: #333;"><label for="price_certificate">@lang('labels.frontend.course.cert_price')  <span style="color: #4f4d96;"> {{$appCurrency['symbol'].' '.$course->cert_price}}</span></label></h5>
                        @if(auth()->check() && (auth()->user()->hasRole('student')) && (Cart::session(auth()->user()->id)->get( $course->id)))
                        <button class="btn genius-btn btn-block text-center my-2 text-uppercase  btn-success text-white bold-font" type="submit">@lang('labels.frontend.course.added_to_cart')
                        </button>
                        @elseif(!auth()->check())
                        @if ($course->is_expired)
                        <div class="subs-form relative-position">
                            <form method="POST" action="{{route('flagemail', $course->id)}}">
                                @csrf
                                <div class="col-md-12">
                                    <label for="email">{{__('validation.attributes.frontend.email')}}</label>
                                    <input type="email" name="email" class="email" id="email" placeholder="{{__('validation.attributes.frontend.email')}}" required="">
                                    <div class="nws-button text-center  gradient-bg text-uppercase" style="position: relative;width: 100%;">
                                        <button type="submit" value="Submit">{{__('global.flagemail.notify_me')}}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                        @else
                            @if($course->free == 1)
                            <a id="openLoginModal" class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font" data-target="#myModal" href="#">@lang('labels.frontend.course.get_now') <i class="fas fa-caret-right"></i></a>
                            @else
                            <a id="openLoginModal" class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font" data-target="#myModal" href="#">@lang('labels.frontend.course.buy_now') <i class="fas fa-caret-right"></i></a>

                            <a id="openLoginModal" class="genius-btn btn-block my-2 bg-dark text-center text-white text-uppercase " data-target="#myModal" href="#">@lang('labels.frontend.course.add_to_cart') <i class="fa fa-shopping-bag"></i></a>

                            <a id="openLoginModal" class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font" data-target="#myModal" href="#">@lang('labels.frontend.course.subscribe')</a>  
                        @endif
                        ...
                        <div class="subs-form relative-position">
                            <form method="POST" action="{{route('flagemail', $course->id)}}">
                                @csrf
                                <div class="col-md-12">
                                    <input type="hidden" name="email" class="email" id="email" value="{{auth()->user()->email}}" placeholder="{{__('validation.attributes.frontend.email')}}" required="">
                                    <div class="nws-button text-center  gradient-bg text-uppercase" style="position: relative;width: 100%;">
                                        <button type="submit" value="Submit">{{__('global.flagemail.notify_me')}}</button>
                                    </div>
                                </div>
                        ...
                        @else 
                            ...
                            <form action="{{ route('cart.getnow') }}" method="POST">
                                ...
                                <input type="hidden" name="inc_certificate" class="inc_certificate" value="0">
                                <button class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font" href="#">@lang('labels.frontend.course.get_now') <i class="fas fa-caret-right"></i></button>
                            </form>
                            @else
                            <form action="{{ route('cart.checkout') }}" method="POST">
                                ...
                                <input type="hidden" name="amount" value="{{($course->free == 1) ? 0 : $course->price}}" />
                                <input type="hidden" name="inc_certificate" class="inc_certificate" value="0">
                                <button class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font" href="#">@lang('labels.frontend.course.buy_now') <i class="fas fa-caret-right"></i></button>
                            </form>
                            ...
                            <form action="{{ route('subscription.course_subscribe') }}" method="POST">
                                ...
                                <input type="hidden" name="amount" value="{{($course->free == 1) ? 0 : $course->price}}" />
                                <input type="hidden" name="inc_certificate" class="inc_certificate" value="0">
                                <button type="submit" class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font">
                                    @lang('labels.frontend.course.subscribe')</button>
                            </form>
                            @else
                            <a class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font" href="{{ route('subscription.plans') }}">@lang('labels.frontend.course.subscribe')</a>
                            @endif
                            @endif
                        @endif
                        ...
                        <a href="{{route('lessons.show',['course_id' => $course->id,'slug'=>$continue_course->model->slug])}}" class="genius-btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font">
                    ...
                    <div class="couse-feature ul-li-block">
                        <ul>
                            ...
                            <li> @lang('labels.frontend.course.category') <span><a href="{{route('courses.category',['category'=>$course->category->slug])}}" target="_blank">{{$course->category->name}}</a> </span></li>
                    ...
                    <div class="side-bar-widget">
                        ...
                        <div class="latest-news-posts">
                            ...
                            <div class="latest-news-area">
                                ...
                                <h3 class="latest-title bold-font"><a href="{{route('blogs.index',['slug'=>$item->slug.'-'.$item->id])}}">{{$item->title}}</a>
                                </h3>
                            </div>
                            <!-- /post -->
                            @endforeach
...
<script>
    ...
    $(function(){
        $(document).on('change', '#inc_certificate_', function(){
            if($(this).prop('checked')){
                $(".inc_certificate").val(1)
            }else{
                $(".inc_certificate").val(0);
            }
        })
    })
</script>
